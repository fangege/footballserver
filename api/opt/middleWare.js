...
async function autho(req, res,next) {
    ...
    try {
        ...
        result = await conn.queryAsync("select b.accountid,b.type from  t_token a left join t_account b on a.userid=b.accountid where a.token=? and a.domain=1 and b.status=1", [token]);
        if(result.length==0){ 
            return notValidLogin(req,res);
        }
